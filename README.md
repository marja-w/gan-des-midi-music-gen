...
## [MM GAN DES](experimenting%20with%20adjsims%20MIDI%20VERSION) # TODO: update name and replace links with new name

Code for training and using the multi-modal GAN (MM-GAN) with Discrete Event Simulator (DES).

### [adj_sim_outputs](experimenting%20with%20adjsims%20MIDI%20VERSION/adj_sim_outputs)

The output files created during model training or when using the DES to generate music. The MIDI format is stored.

### [models](experimenting%20with%20adjsims%20MIDI%20VERSION/models)

Folder where the model files are stored during training. 

### [datasets](experimenting%20with%20adjsims%20MIDI%20VERSION/datasets.py)
...
### [matrix_sim_process](experimenting%20with%20adjsims%20MIDI%20VERSION/matrix_sim_process.py)
...
### [sim_log_to_midi.py](experimenting%20with%20adjsims%20MIDI%20VERSION/sim_log_to_midi.py)
...
### [SIMNN](experimenting%20with%20adjsims%20MIDI%20VERSION/SIMNN.py)
...
### [simulation_v3](experimenting%20with%20adjsims%20MIDI%20VERSION/simulation_v3.py)

Contains the actual DES. Is used by the function ``matrix_to_midi()`` to perform the simulation, after receiving the matrix
generated by the GAN generator. 

### [util](experimenting%20with%20adjsims%20MIDI%20VERSION/util.py)
