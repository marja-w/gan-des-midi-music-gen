...
class MultiModalGAN(nn.Module):
    ...
    def forward(self, noise1, noise2, input_tensor, count, make_dot_png=True):
        gen_output1 = self.generator1(noise1)
        gen_output2 = self.generator2(noise2, input_tensor)

        if make_dot_png:
            # Create a dot file
            dot = make_dot(gen_output1, params=dict(self.generator1.named_parameters()), show_attrs=True, show_saved=True)

            # Save the dot file as a PNG
            dot.format = 'png'
            dot.render('output_graph_generator')
...
class TestMultiModalGAN(unittest.TestCase):
    def test_training_loop(self, batch_size=16):
        ...
        start = 100 # start of the sequence NOTE: CHANGING THIS VALUE DRASTICALLY CHANGES THE OUTPUT. SIMULATOR NEEDS TIME TO GET INTO A GOOD STATE
        ...
        for epoch in range(num_epochs):
            ...
            for i, (piano_roll, durations, beats) in enumerate(train_loader):
                ...
                # Train Discriminator
                disc_opt.zero_grad()
                fake_output, failed_sim_count = mmgan(noise1, noise2, beats, count, make_dot_png)
                if make_dot_png:
                    # Create a dot file
                    dot = make_dot(fake_output, params=dict(mmgan.named_parameters()), show_attrs=True, show_saved=True)
