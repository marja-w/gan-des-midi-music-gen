...
curr_dir = os.getcwd()
if "GAN_DES" not in curr_dir:
    os.chdir(f"{os.getcwd()}/GAN_DES")
print(os.getcwd())
...
def generate_song(model_folder):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # load model
    gen = Generator()
    gen.load_state_dict(torch.load(model_folder))
    gen.eval()
    # generate the random noise
    fake_noise = get_noise(n_samples=1, noise_dim=100)
    # generate the fake images by passing the random noise to the generator
    adj_matrix = gen(fake_noise)  # matrix of size (batch_size, 1 , 20, 20)
    # convert generated tensors to list of numpy matrices
    adj_matrix = adj_matrix.squeeze().detach().cpu().numpy()

    generated_specs = matrix_to_wav(adj_matrix, start=0, end=216,
...
if __name__ == '__main__':
    # Load Data
    batch_size = 30  # number of training samples in one training step

    # input_song = InputSong(audio_file="./data/classical.00000.wav")

    MODEL_PATH = "models/"
    input_song = MaestroDataset(batch_size=batch_size)
    dataloader = DataLoader(input_song, batch_size=1, shuffle=True, collate_fn=my_collate)
    ...
    # train params
    n_epochs = 1  # number of epochs (one full train iteration)
    cur_step = 0
    mean_generator_loss = 0
    mean_discriminator_loss = 0
    display_step = 5
    save_step = 5  # per what number of step the model is stored in /models
    z_dim = 100  # dimension of the random noise input for generator
    ...
    for epoch in range(n_epochs):
        for real in tqdm(dataloader):  # Dataloader returns the batches
            ...
            fake = matrix_to_wav(fake, start=0, end=216,
                                 device=device, overwrite=True)  # build simulator from matrix and get spectrograms from created audio
            ...
            if cur_step % save_step == 0 and cur_step > 0:
                ...
                torch.save(gen.state_dict(),
                           f"{MODEL_PATH}/gen_{cur_step}_{datetime.timestamp(now)}.pt")  # save generator
            cur_step += 1
